...
class UncertaintyDetector:
    ...
    def detect(self, probabilities: Dict[str, float], multi_label: bool = False) -> UncertaintyResult:
        ...
        if multi_label:
            confident_count = sum(1 for value in values if value >= self.low_confidence)
            if confident_count == 0:
                return UncertaintyResult(True, entropy, "no_confident_multi_label")
            return UncertaintyResult(False, entropy, "confident_multi_label")
